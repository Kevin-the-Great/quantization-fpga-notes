...
最近增补：2026-09-16；此前条目的核对日期为 2026-09-14。DiTPA 目前仅核对官方摘要、出版信息及作者代码，全文待补；各项具体查阅范围见笔记。个人精读和复现状态由读者自行记录。本表属于 FPGA 架构主线，比较计算复用、数据流和存储组织；Allo 等 HLS 资料作为架构实现与映射方法的支撑参考。模型类型作为适用条件。VLA 加速器仍处于设计前的研究阶段。

## 2026-09-16 优先候选：DiTPA（ISCA 2026）

[DiTPA 阅读笔记](../notes/ditpa-2026.md)与[组会提纲](../talks/ditpa-2026.md)已建立。它从动作规划中的动作、去噪和多模态冗余组织专用加速架构，收在本主线作为可迁移方法参考；具体实现平台及全文实验仍待核实。[机构摘要与出版记录](https://researchportal.hkust.edu.hk/en/publications/ditpa-a-dit-based-action-planner-accelerator-exploiting-action-de/)

| 比较问题 | 本次得到的依据 | 对当前研究的意义 |
|---|---|---|
| 缓存与调度应该围绕什么组织？ | 摘要给出三层冗余及对应硬件模块；具体判定、容量和恢复条件待全文 | 与 HG-PIPE 的流水和缓冲作定向比较，先画数据生命周期 |
| 公开性能覆盖什么？ | [软件报告时延](https://github.com/fengbintu/ISCA2026-DiTPA/blob/7be831e7aeb170c992a330ea995e8e0761966418/scripts/ditpa_software_evaluation.py#L174-L186)扣除视觉/语言部分；[公开硬件评估](https://github.com/fengbintu/ISCA2026-DiTPA/blob/7be831e7aeb170c992a330ea995e8e0761966418/README.md#L357-L360)为模拟 | 不能直接对比完整 VLA 的上板端到端时延 |
| 与量化怎样结合？ | 作者代码有反量化和带条件的激活 QDQ；默认完整位宽与硬件精度未核实 | 检查量化误差是否改变复用判定；在相同采样和任务协议下做组合消融 |
