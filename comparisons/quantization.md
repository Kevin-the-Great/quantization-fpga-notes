...
更新：2026-09-14。本文由助理根据一手论文和作者代码仓库整理，属于**初筛与定向查阅笔记**；不表示用户已精读、复现或完成组会汇报。先比较 2026 年新增工作，并补充 2025 年下半年论文；后面的 7 篇经典背景用于追溯方法机制，不构成最新工作全集或 SOTA 排名。
...
## 近期工作：2026 为主，2025 下半年补充

日期取本次核实的 arXiv v1 记录，不保证是全网最早披露；正式收录状态单独列出。各篇笔记保存原文、日期证据、会议证据及章节定位。下面的问题用于组织阅读和后续验证，不代表已经完成迁移实验。

| 论文 / 首次公开 / 状态 | 机制 | 精度与实现边界 | 我们要比较的问题 | 来源笔记 |
| --- | --- | --- | --- | --- |
| ReSpinQuant / 2026-04-13 / ICML 2026 官方收录 | 逐层学习旋转并折叠到权重，以低秩修正处理残差基底不一致 | 主配置 W4A4KV4、W3A3KV3；仍需校准优化与在线残差修正 | 相比共享旋转，逐层适配能带来多少收益，新增修正是否值得实现？ | [原文与证据](../notes/respinquant-2026.md) |
| OrbitQuant / 2026-07-02 / 预印本 | 归一化与随机置换分块 Hadamard，配合同维度共享的 Lloyd–Max 码本 | W4A4/W2A4 是非均匀码本表示；免数据校准仍保留在线范数、旋转及码本处理 | 省去范围校准后，码本和在线变换的成本是否适合目标硬件？ | [原文与证据](../notes/orbitquant-2026.md) |
| MXFP PTQ Benchmark / 2026-01-14 / ACL 2026 正式发表 | 对照不同 PTQ 算法，分离共享尺度误差并分析预缩放 | MXFP W8A8/W4A8/W4A4；不能与 INT4、NVFP4 只按位宽等同 | 在一致模型与协议下，算法、粒度和数值格式分别造成什么差异？ | [原文与证据](../notes/mxfp-ptq-benchmark-2026.md) |
| 6Bit-Diffusion / 2026-03-19 / 预印本 | 根据前一步块变化预测敏感性，动态切换精度，并结合残差缓存 | 权重存 NVFP4，激活走 NVFP4/INT8；不是统一六位数据通路；总收益包含跳算 | 少步生成是否仍有稳定的跨步关系，切换与缓存应怎样分别消融？ | [原文与证据](../notes/6bit-diffusion-2026.md) |
| Audio DiT PTQ / 2025-09-30 / WASPAA 2025 官方收录（补充） | 比较静态与分步分通道缩放，加入 SVD 低秩误差补偿 | 补偿分支为 FP16；原文正文 W4A8 与 Table 1 的 W4A4 标注不一致，暂只收定性机制 | 分步尺度能否通过离线表保留精度，同时减少动态处理成本？ | [原文与证据](../notes/audio-dit-ptq-2025.md) |

## 2026 年 VLA 量化：比较目标与覆盖范围

这组用于把通用量化问题联系到动作任务；与前面的 LLM、图像、视频和音频方法共同阅读。分别依据各篇原文核对覆盖范围，不直接采用另一篇论文对它的概括。

| 工作 / 状态 | 方法关注什么 | 位宽标签与保留部分 | 组会比较问题与原文 |
|---|---|---|---|
| HoloQ-VLA / 2026 预印本；8 月更新 v3 | 复合旋转与逐时间步静态尺度 | 报告语言和扩散动作模块的 W4A4，含注意力；辅助算子及实际内核精度仍需另核实 | 改变采样步数后尺度是否仍有效？对照跨步统计聚合。[v3 正文](https://arxiv.org/html/2605.28803v3) · [笔记](../notes/holoq-vla-2026.md) |
| QuantVLA / CVPR 2026 | 校准注意力温度及残差输出能量 | 选择性 W4A8；DiT Q/K/V/O 投影保留浮点 | 接口分布校准怎样补充层输出重构？扩大覆盖范围后能否保留收益？[§3.2–3.3](https://arxiv.org/html/2602.20309v4) · [笔记](../notes/quantvla-2026.md) |
| QVLA / ICLR 2026 | 根据动作敏感性分配通道位宽 | 同时有仅权重和权重/激活实验；W4 是平均预算，projector 与动作头保持 BF16 | 任务敏感性与层级 MSE 如何对照？通道混合精度是否适合统一矩阵核？[§3–4](https://arxiv.org/html/2602.03782v1) · [笔记](../notes/qvla-2026.md) |

统一位宽、平均位宽和选择性覆盖应先对齐再比较结果；这里未把三篇排成同一量化配置的成功率榜。

## 与 FPGA 架构交叉的量化方法

| 工作 | 数值表示如何改变架构问题 | 阅读入口 |
|---|---|---|
| ViM-Q / FCCM 2026 | APoT 编码及动态激活尺度要求查表和量化器配合；不能只按 W4A8 标签比较普通整数乘法 | [方法与证据](../notes/vim-q-2026.md) |
| HGQ-LUT / FCCM 2026 | 细粒度位宽和剪枝与逻辑资源目标联合训练，改变实现的函数与展开成本 | [方法与证据](../notes/hgq-lut-2026.md) |
| LUT-LLM / FCCM 2026；2025 年预印本 | 向量联合量化要求码本训练、质心搜索与存储查表；表项位宽不是标量 W/A 位宽 | [方法与证据](../notes/lut-llm-2026.md) |
| QUARK / ICCAD 2025 | 量化与整数近似促成非线性子算子共享，需共同检查误差、尺度对齐和并发性 | [方法与证据](../notes/quark-2025.md) |

架构侧的证据与条件见 [FPGA 横向比较](architectures.md)。

## 经典背景

下面三组保留重构、缩放、旋转和迭代校准的基础比较，用来解释近期方法改变了哪一部分。
